...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate deterministic GFS + ECMWF Day-2 outlook")
    p.add_argument("--data-dir", default="data/multimodel-day2")
    p.add_argument("--output-dir", default="output/multimodel-day2")
...
def main() -> None:
    ...
    gfs = GFSAdapter(); ecmwf = ECMWFAdapter()
    cycle = min(gfs.latest_cycle(), ecmwf.latest_cycle())
    gh = set(gfs.discover_forecast_hours(cycle)); eh = set(ecmwf.discover_forecast_hours(cycle))
    hours = [h for h in requested if h in gh and h in eh]
    if not hours:
        raise RuntimeError("No common GFS/ECMWF Day-2 forecast hours")
    ...
    for name, ds in model_fields.items():
        ds.to_netcdf(output / f"{name.lower()}_day2_v3.nc")

    consensus, summary = multimodel_consensus(model_fields)
    consensus.to_netcdf(output / "multimodel_day2_consensus_v1.nc")

    maxima = {name: {k: _safe_max(ds[k]) for k in ("severe", "tornado", "hail", "wind")} for name, ds in model_fields.items()}
    ...
    manifest = {
        ...
        "valid_start": (cycle + timedelta(hours=24)).isoformat(),
        "valid_end": (cycle + timedelta(hours=48)).isoformat(),
        ...
        "calibrated": False,
        "maxima": maxima,
        "warning": "Day-2 engineering guidance. Historical skill weighting not yet calibrated.",
    }
