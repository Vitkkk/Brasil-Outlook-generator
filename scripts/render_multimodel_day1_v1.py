...
def category(ax, ds, cfg):
    ...
    ax.legend([Patch(facecolor=CAT_COLORS[c],edgecolor="#555",label=CAT_NAMES[c]) for c in reached],
              [CAT_NAMES[c] for c in reached],loc="lower right",ncol=3,fontsize=7,title="Risk Category",title_fontsize=8,framealpha=.95)
...
def hazard(ax, da, thresholds):
    ...
    if reached:
        ax.legend([Patch(facecolor=HAZARD_COLORS[t],edgecolor="#555",label=f"{int(t*100)}%") for t in reached],
                  [f"{int(t*100)}%" for t in reached],loc="lower right",ncol=2,fontsize=7,
                  title="Probability within 40 km",title_fontsize=8,framealpha=.95)
